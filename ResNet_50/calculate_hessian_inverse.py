...
import LOBS.ResNet_50.models as models
from LOBS.ResNet_50 import dataset
...
def load_model(name):
	# 根据给定的名字创建并返回一个模型的实例，这个模型有一个喂数据的单节点.
...
for layer_name in layer_name_list:
	...
	with tf.Session() as sess:
		...
		for batch_count, (labels, images) in enumerate(image_producer.batches(sess)):
			# Get layer inputs for this layer
			layer_inputs = sess.run(get_layer_inputs_op[layer_name], feed_dict={input_node: images})
			# print layer_inputs.shape
			if layer_name.startswith('conv'):
				if batch_count == 0:
					hessian = sess.run(get_hessian_conv_op, feed_dict={layer_inputs_holder: layer_inputs})
				else:
					hessian += sess.run(get_hessian_conv_op, feed_dict={layer_inputs_holder: layer_inputs})
			elif layer_name.startswith('res'):
				if batch_count == 0:
					hessian = sess.run(get_hessian_res_op, feed_dict={layer_inputs_holder: layer_inputs})
				else:
					hessian += sess.run(get_hessian_res_op, feed_dict={layer_inputs_holder: layer_inputs})
			elif layer_name.startswith('fc'):
				if batch_count == 0:
					hessian = sess.run(get_hessian_fc_op, feed_dict={layer_inputs_holder: layer_inputs})
				else:
					hessian += sess.run(get_hessian_fc_op, feed_dict={layer_inputs_holder: layer_inputs})
			# See watch info
